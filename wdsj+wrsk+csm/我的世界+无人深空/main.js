...
const Game = (() => {
  ...
  function init() {
    ...
    // 初始星球世界 (每颗星球独立场景, 由 Worlds 管理)
    World = Worlds.get(0);
    planetScene = World.scene;

    Player.init(camera, planetScene);
    Ship.build(planetScene);
    Ship.setCamera(camera);
    Portals.init(renderer, camera);
  ...
  function applyPlanetAtmosphere(config) {
    ...
    World.scene.fog = new THREE.Fog(fogColor, 30, 150);
    World.scene.background = fogColor.clone().lerp(new THREE.Color(config.sky), 0.6);
    World.ambLight.intensity = 0.45 + config.ambient * 0.15;
    UI.setPlanetInfo(config.name, config.type + ' · ' + config.hazardName);
    UI.setEnvReadout(config.hazardName + ' · 重力 1.02g');
  }

  // ---------- 世界切换 (飞行进入大气 / 传送门跨星球) ----------
  function switchWorld(idx, viaPortal = false) {
    const changed = !World || World.planetIdx !== idx;
    World = Worlds.get(idx);
    planetScene = World.scene;
    currentPlanetIdx = idx;
    applyPlanetAtmosphere(World.planet);
    Player.moveToScene(planetScene);
    if (changed) {
      Sfx.stopLoop('wind');
      Sfx.windLoop(World.planet.hazardRate > 1.5 ? 1.6 : 1);
      Missions.onVisitPlanet(idx);
    }
    if (viaPortal && changed) {
      UI.notify(World.planet.name, World.planet.type + ' · 穿过传送门抵达');
    }
  }
  ...
  async function enterAtmosphere(planetEntry) {
    ...
    // 切换到星球场景
    switchWorld(planetEntry.index);
    const config = planetEntry.config;
    planetScene.add(Ship.group);
    shipPlanetIdx = planetEntry.index;
  ...
  function landShip() {
    ...
    Ship.land(() => {
      state = 'planet';
      shipPlanetIdx = currentPlanetIdx;
      spawnDust(Ship.state.pos.x, Ship.state.pos.y - 0.8, Ship.state.pos.z);
  ...
  function spawnSmoke(x, y, z, fire = false) {
    ...
    planetScene.add(m);
    particles.push({
      mesh: m, scene: planetScene, life: fire ? 0.5 : 1.8 + Math.random(),
  ...
  function spawnDust(x, y, z) {
    for (let i = 0; i < 14; i++) {
      ...
      planetScene.add(m);
      particles.push({
        mesh: m, scene: planetScene, life: 1 + Math.random(),
  ...
  function spawnBreakParticles(x, y, z, blockId, scene = null) {
    const targetScene = scene || planetScene;
    ...
    for (let i = 0; i < n; i++) {
      ...
      targetScene.add(m);
      particles.push({
        mesh: m, scene: targetScene, life: 0.7 + Math.random() * 0.4,
  ...
  function updateParticles(dt) {
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      if (p.life <= 0) {
        (p.scene || planetScene).remove(p.mesh);
        particles.splice(i, 1);
      }
    }
    // 坠毁飞船冒烟 (未修复时)
    if (state === 'planet' && !Ship.allRepaired() && shipPlanetIdx === currentPlanetIdx) {
      smokeTimer -= dt;
      if (smokeTimer <= 0) {
        smokeTimer = 0.12;
        const s = Ship.state.pos;
  ...
  function updateInteract() {
    ...
    if (shipPlanetIdx !== currentPlanetIdx) { UI.interactTip(null); return; }
  ...
  function interact() {
    if (state === 'planet') {
      if (shipPlanetIdx !== currentPlanetIdx) return;
      const d = Player.pos.distanceTo(Ship.state.pos);
      if (d < 6) boardShip();
    } else if (state === 'atmo') {
  ...
  function bindEvents() {
    ...
    document.addEventListener('mousedown', e => {
      ...
      if (e.button === 0) {
        if (Portals.gunActive) Portals.fire('blue');
        else Player.startMine();
      }
      if (e.button === 2) {
        if (Portals.gunActive) Portals.fire('orange');
        else Player.placeBlock();
      }
    });
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.code === 'KeyQ' && state === 'planet') { Portals.toggleGun(); return; }
      if (e.code === 'Digit1' && state === 'planet') { if (Portals.gunActive) Portals.toggleGun(false); return; }
      if (e.code === 'Digit2' && state === 'planet') { if (!Portals.gunActive) Portals.toggleGun(true); return; }
  ...
  function animate() {
    ...
    if (state === 'planet') {
      ...
      // 飞船标记 (仅当飞船在当前星球)
      if (Missions.index <= 7) {
        if (shipPlanetIdx === currentPlanetIdx) {
          if (!UI.hasMarker('ship')) UI.addMarker('ship', '星舰', '▲', 'amber');
          UI.updateMarker('ship', Ship.state.pos.clone().add(new THREE.Vector3(0, 3, 0)), camera, Player.pos);
        } else {
          UI.removeMarker('ship');
        }
      }
      Portals.render(renderer, planetScene, camera);
    }
  ...
  return {
    ...
    get currentPlanetIdx() { return currentPlanetIdx; },
    requestPointerLock, spawnBreakParticles, switchWorld,
  };
})();
