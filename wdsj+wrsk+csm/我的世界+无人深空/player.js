...
const Player = (() => {
  ...
  function moveToScene(scene) {
    ...
    if (sceneRef) { sceneRef.remove(beamLine); sceneRef.remove(beamGlow); }
    scene.add(beamLine);
    scene.add(beamGlow);
    sceneRef = scene;
  }
    ...
    for (let x = minX; x <= maxX; x++)
      for (let y = minY; y <= maxY; y++)
        for (let z = minZ; z <= maxZ; z++) {
          ...
          if (typeof Portals !== 'undefined' && Portals.isPassable(x, y, z)) continue;
          if (axis === 'y') {
  ...
  function updateMining(dt) {
    ...
    const dir = getDir();
    const hasPortals = typeof Portals !== 'undefined';
    let hit = null, hitWorld = World, cast = null;
    if (hasPortals) {
      cast = Portals.castThrough(World, eye, dir, 6);
      hit = cast.hit;
      hitWorld = cast.world;
    } else {
      hit = World.raycast(eye, dir, 6);
    }

    if (miningActive && stats.beam > 0.02 && hit) {
      const def = BLOCKS[hit.block];
      if (!mining || mining.x !== hit.x || mining.y !== hit.y || mining.z !== hit.z || mining.world !== hitWorld) {
        mining = { x: hit.x, y: hit.y, z: hit.z, world: hitWorld, progress: 0, hardness: def.hard || 1, block: hit.block };
      }
      ...
      // 光束视觉 (首段在当前世界, 穿门后的远端段交给 Portals 绘制)
      const seg0 = cast ? cast.segments[0] : null;
      const localEnd = seg0 ? seg0.b.clone() : eye.clone().addScaledVector(dir, hit.dist - 0.05);
      const finalPoint = cast ? cast.pos : localEnd;
      const muzzle = eye.clone()
        .add(dir.clone().multiplyScalar(0.8))
        .add(new THREE.Vector3(Math.cos(yaw) * 0.32, -0.28, -Math.sin(yaw) * 0.32));
      beamLine.geometry.setFromPoints([muzzle, localEnd]);
      beamLine.visible = true;
      if (hitWorld === World) {
        beamGlow.position.copy(finalPoint);
        beamGlow.scale.setScalar(0.8 + Math.random() * 0.5);
        beamGlow.visible = true;
        if (hasPortals) Portals.clearBeamPath();
      } else {
        beamGlow.visible = false;
        if (hasPortals) Portals.setBeamPath(cast.segments, finalPoint, hitWorld);
      }

      if (mining.progress >= 1) {
        const drop = def.drop;
        mining.world.setBlock(mining.x, mining.y, mining.z, 0);
        ...
        Game.spawnBreakParticles(mining.x + 0.5, mining.y + 0.5, mining.z + 0.5, mining.block, mining.world.scene);
        mining = null;
      }
    } else {
      mining = null;
  ...
  function placeBlock() {
    ...
    const eye = new THREE.Vector3(pos.x, pos.y + EYE, pos.z);
    let hit = null, hitWorld = World;
    if (typeof Portals !== 'undefined') {
      const cast = Portals.castThrough(World, eye, getDir(), 6);
      hit = cast.hit;
      hitWorld = cast.world;
    } else {
      hit = World.raycast(eye, getDir(), 6);
    }
    if (!hit) return;
  ...
  return {
    ...
    moveToScene, syncCamera,
    get pos() { return pos; }, get vel() { return vel; },
