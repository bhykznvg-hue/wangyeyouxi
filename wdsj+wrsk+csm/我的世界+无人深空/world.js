...
function makePlanetConfig(seed, archetypeIdx = null) {
...
function createWorld() {
  ...
  let anchors = [];             // 额外保活锚点 [{x, z, r}] (传送门端点等)

  function makePlanet(seed, archetypeIdx = null) { return makePlanetConfig(seed, archetypeIdx); }
  ...
  let buildQueue = [];
  function setAnchors(list) { anchors = list || []; }

  function update(centerX, centerZ, budgetMs = 8) {
    const ccx = Math.floor(centerX / CHUNK), ccz = Math.floor(centerZ / CHUNK);
    // 保活中心: 玩家/飞船 + 锚点 (传送门端点)
    const centers = [{ cx: ccx, cz: ccz, r: RENDER_DIST }];
    anchors.forEach(a => centers.push({ cx: Math.floor(a.x / CHUNK), cz: Math.floor(a.z / CHUNK), r: a.r || 3 }));
    // 需要的区块
    const needed = new Set();
    centers.forEach(c => {
      for (let dx = -c.r; dx <= c.r; dx++)
        for (let dz = -c.r; dz <= c.r; dz++) {
          if (dx * dx + dz * dz > (c.r + 0.5) ** 2) continue;
    ...
    chunks.forEach((e, key) => {
      if (!needed.has(key)) {
        ...
        const keep = centers.some(c => Math.abs(cx - c.cx) <= c.r + 2 && Math.abs(cz - c.cz) <= c.r + 2);
        if (!keep) {
          if (e.mesh) { scene.remove(e.mesh); e.mesh.geometry.dispose(); }
          if (e.crossMesh) { scene.remove(e.crossMesh); e.crossMesh.geometry.dispose(); }
          chunks.delete(key);
  ...
  return {
    ...
    makePlanet, heightAt: (x, z) => heightAt(x, z), setAnchors,
    get planet() { return planet; }, CHUNK, HEIGHT, PLANET_ARCHETYPES,
    get pendingBuilds() { return buildQueue.length; },
    // 由 Worlds 管理器填充
    scene: null, ambLight: null, sunLight: null, planetIdx: -1,
  };
}

// ---------- 当前激活的世界 (由 main.js / Portals 切换) ----------
var World = null;

// ---------- 世界管理器: 每颗星球一个持久实例, 各自拥有独立场景 ----------
const Worlds = (() => {
  const cache = new Map();   // planetIdx -> world 实例
  ...
  function get(idx) {
    ...
    if (w) return w;
    w = createWorld();
    const config = Space.planets[idx].config;
    const scene = new THREE.Scene();
    const fogColor = new THREE.Color(config.fog);
    scene.fog = new THREE.Fog(fogColor, 30, 150);
    scene.background = fogColor.clone().lerp(new THREE.Color(config.sky), 0.6);
    const amb = new THREE.AmbientLight(0xffffff, 0.45 + config.ambient * 0.15);
    scene.add(amb);
  ...
  function has(idx) { return cache.has(idx); }

  return { get, has, makePlanet, get cache() { return cache; } };
})();
